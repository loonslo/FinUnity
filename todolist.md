...
**完成记录（2026-06-22）**：全部返回页已收口到 `FinTopBar`，文字灰度与盈亏色收口到 `FinColors`，删除无引用组件，`formatCurrency` 保持单一实现并接入全局金额隐藏。`assembleDebug` + `testDebugUnitTest` + Android 测试源码编译均通过。
...
- [x] **交易流水自然语言文案**：`MainViewModel.kt` 中 `note = "资产记录买入: X"` 等改为自然语言（如"买入 X · 1000 元"）。
      验收：流水页无"资产记录买入"这类程序化文案，类型/金额/数量单价/时间/备注齐全可读。
- [x] **Onboarding 持久化**：`Settings` 加 `val onboarded: Boolean = false`（DB 版本 7→8 迁移：`ALTER TABLE settings ADD COLUMN onboarded INTEGER NOT NULL DEFAULT 0`，登记到 `DatabaseMigrations.ALL_MIGRATIONS`）；完成首个账户后置 true；`MainScreen` 用 `onboarded` 而非"无账户"判断是否展示引导。
      验收：完成首个账户后不再展示引导；清空账户也不强制重走。
- [x] **README 更新**：补四象限说明、每日更新、新增页面、`gradlew.bat`/JAVA_HOME 构建说明。
      验收：README 与实际功能一致，新人按文档可在 Windows `./gradlew assembleDebug` 成功。
- [x] **（可选）首页最近变化**：总览页底部可加一条"最近变化"摘要（数据用 `HistoryRepository.getMonthlyChange()`）。低优先，做不做取决于首页是否过空。
...
**信任与资金安全**
- [x] 删除前展示影响范围：`AssetRecordScreen`/账户删除确认框列出将删除的"资产记录数 / 流水数 / 价格历史数"。
      验收：删除对话框显示具体受影响条数，而非仅一句"确定删除吗"。
- [x] 金额隐藏全局化：把 `AccountHubScreen` 的 `amountsVisible` 提升为全局状态（`MainViewModel` 或 `Settings`），覆盖首页/详情/流水，并记住选择。
      验收：任一页切换"隐藏金额"后，所有页同步隐藏并在重启后保持。
- [x] 卖出流程补齐：`AssetRecordScreen.showSellDialog` 增加成交价、费用、日期、备注（当前只填数量、用现价估算）。
      验收：卖出流水含真实成交价/费用/日期，不再仅按当前价估算。
- [x] 转账余额预览：`CashFlowScreen` 转账时显示"转出后余额/转入后余额"，跨币种时解释为何禁用。
      验收：转账前可见双方余额变化预览。

**文案与解释（少刺激、多解释，对照设计约束的文案原则）**
- [x] 去掉"实时价格"绝对表述：扫描 UI/`README.md`/`CLAUDE.md`，将"实时/每5分钟"改为"自动同步 / 每日更新 / 最近同步"。
      验收：全仓无"实时价格"字样，表述与"每日更新一次"一致。
- [x] 风险桶"旧持仓"改名：`RiskBucketDetailScreen` 的"旧持仓"改为"证券持仓（迁移数据）"。
      验收：界面不再出现"旧持仓"这种内部术语。
- [x] 设置项影响说明：`SettingsScreen`/`TargetAllocationScreen` 注明"本位币/目标配置/阈值改动会影响哪些页面"。
      验收：每个设置项下有一句影响说明。
- [x] 错误提示给行动建议：价格同步失败等提示补"稍后重试 / 手动输入价格 / 查看上次价格"。
      验收：错误提示是可操作建议而非技术报错。

**录入与流水体验**
- [x] 表单离开前未保存提示：`AccountScreen`/`AssetRecordScreen` 返回时若有改动，弹"保存草稿 / 放弃修改"。
      验收：有未保存输入时返回会二次确认。
- [x] 大额输入辅助：金额输入框下显示"约 X 万 CNY"。
      验收：输入 100000 时显示"约 10.00 万"。
- [x] 交易流水筛选：`TransactionHistoryScreen` 增加按账户/资产/类型/时间范围筛选。
      验收：可按四个维度过滤流水。

**交互去重**
- [x] 资产详情主操作收敛：`AssetDetailScreen` 顶部"买入/卖出" 与底部"调整持仓"重复，统一为主操作"记一笔"+二级"编辑资料/删除"。
      验收：详情页无重复的买卖入口。
- [x] `CashFlowScreen` 的"投资买卖统一接入这里"半成品提示：要么接入真实买卖，要么先隐藏该入口。
      验收：无"功能像坏了"的占位入口。
- [x] `AccountHubScreen` 的"加权收益率/年化收益率"若仍显示 `--`：补齐计算或隐藏。
      验收：界面无长期 `--` 占位指标。
...
**完成记录（2026-06-22）**：总览风险象限仍包含锁定专款；策略配置比例、再平衡建议、规划页和月度复盘的偏离金额均已改用 `strategyAssets`，并补充锁定专款排除的回归测试。
...
**完成记录（2026-06-22）**：新增 `AssetRecord.industryTag` 及 v11→v12 迁移，备份升至 v4；风险体检已按可投策略盘检查单只持仓10%、行业20%、训练仓6万、黄金5%及个股亏损10%/15%，锁定专款排除在外；补充纯函数单测与 Room 持久化集成测试。
...
**完成记录（2026-06-22）**：口径确定为 `asset_snapshots.totalAssets` 历史高点；快照少于 2 条时仅提示等待积累。规划页已接入 -5/-10/-15/-20/-25% 档位，显示定投倍数、资金来源及按「弹药」落点现值分批计算的建议金额；补充档位、样本不足和未触发单测。
...
**完成记录（2026-06-22）**：新增规划页入口与 `StressTestScreen`，M 采用页面输入；内置 2022/2020/2008/2000 四个情景，可切换并编辑四象限跌幅。浮亏按「可投策略盘 × Σ(象限权重 × 跌幅)」计算，逐情景对比 M 并给出结构调整提示；已补加权计算与超限单测。
...
**完成记录（2026-06-22）**：月度复盘页新增月/季/年切换的结构化 checklist。防守充足度、风险仓位上限、落点超限、训练仓6万、弹药余额与风险警告由当前数据自动判定；境外只增不减、目标复核、年度压测等保留本次手动勾选。
...
**完成记录（2026-06-22）**：账户/象限偏离沿用已完成的策略盘再平衡；新增 `purchaseRestricted` 手动限购标记、v12→v13 迁移及备份 v5。风险体检会汇总 USD 计价的股票/ETF/基金原币市值，在 $45K/$50K/$60K 档位分级提醒（并明确建议根据居住地咨询专业人士）；被标记限购的标的会提示暂停加仓及替代行动。
...
**完成记录（2026-06-22）**：新增 PE、股息率、场内溢价率手动字段、v13→v14 迁移及备份 v6，表单明确标注数据不自动抓取。溢价率≥3% 提示不追高；红利落点/名称在 PE>15 或股息率<3% 时提示估值未达门槛；币种敞口复用 TD-6 的 USD 原币市值汇总。

> 验收沿用全局约定：每项完成后 `./gradlew assembleDebug` 与 `./gradlew testDebugUnitTest` 必须绿；纯逻辑（红线/缺口/压测/回撤）抽成可单测的纯函数。

#### ⬜ P0-截图识别 OCR 接入
**目标**：把截图导入从当前交互原型升级为本地 OCR 流程，识别证券名称、编码、数量和市值，并允许逐项校正后确认导入。
**修改 / 新建文件**：`app/build.gradle`、`AndroidManifest.xml`、`data/repository/OcrRepository.kt`、`ui/screens/PrototypeScreens.kt`、`MainActivity.kt`。
**要求**：优先采用本地 ML Kit Text Recognition；补齐相册/相机权限、识别中/成功/空结果/失败重试状态、低置信度「需确认」标记、人工修改和原图删除选项。确认后统一调用 `MainViewModel.addAssetRecord()`，进入正式编码合并管线；原始截图不上传服务器。
**验收**：识别失败不新增数据；低置信度项必须可修改；成功导入的数据与手动录入走同一套编码合并、成本和收益计算；无网络也可完成本地识别。

#### ⬜ P0-券商授权与定时同步接入
**目标**：把“券商授权导入”从当前数据源入口升级为真实的只读同步能力，覆盖首批选定的平台，并在账户页展示可信的最后同步时间和过期状态。
**前置决策**：确定可合规使用的平台数据接口 / 聚合服务、授权回调方式、凭证的系统级安全存储方案，以及首批平台优先级；不得在客户端保存交易权限或明文凭证。
**要求**：授权、首次拉取、增量同步、失败重试和解除绑定均写入同一份原始持仓数据；同步结果须包含名称、证券编码、数量、成本、市值和归属账户，并进入既有的同码合并管线。
**验收**：授权失败或同步失败不覆盖旧数据；过期数据在账户与总览明确标记；解绑时可选择保留或清除已导入持仓；全程仅申请只读权限。
