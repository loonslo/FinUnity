...
fun buildReviewChecklist(summary: PortfolioSummary): List<ReviewChecklistItem> {
    val defensiveRatio = summary.allocations["DEFENSIVE"] ?: 0.0
    val defensiveTarget = com.finunity.data.local.entity.parseTargetAllocation(summary.targetAllocation)["DEFENSIVE"] ?: 0.0
    ...
    return listOf(
        ReviewChecklistItem("defense", ReviewCadence.MONTHLY, "防守资金是否充足", defensiveRatio + summary.rebalanceThreshold >= defensiveTarget,
            "当前防守 ${(defensiveRatio * 100).toInt()}%，目标 ${(defensiveTarget * 100).toInt()}%"),
        ...
        ReviewChecklistItem("ammo", ReviewCadence.MONTHLY, "回撤弹药是否仍在", ammo > 0.0,
            if (ammo > 0.0) "当前弹药 ${summary.baseCurrency} ${String.format(Locale.US, "%,.0f", ammo)}" else "未找到「弹药」落点"),
