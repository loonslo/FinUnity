    ...
    Scaffold(
        ...
        Column(
            ...
            PolicySection("2. 网络请求") {
                Text("当你主动刷新行情或后台同步运行时，应用会向 Yahoo Finance 请求证券价格和汇率。请求可能包含你填写的证券代码；资产名称、账户余额和交易明细不会上传到衡仓服务器。行情服务的可用性、延迟和准确性不由本应用保证。")
            }
            PolicySection("3. 截图识别") {
                Text("持仓截图使用设备端 OCR 处理。图片由系统文件选择器提供给应用，识别文本后只保存你确认导入的结果；应用不会把截图上传到衡仓服务器。")
            }
